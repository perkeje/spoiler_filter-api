...
module.exports.create = async (
  ...
  try {
    let response = await pool.query(
      "INSERT INTO REVIEWS(user_id,movie,review,is_spoiler,grade) VALUES($1,$2,$3,$4,$5) RETURNING *",
      [user_id, movie, review, is_spoiler, grade]
    );
...
module.exports.get = async (user_id, pool) => {
  let reviews = await pool.query(
    "SELECT * FROM REVIEWS WHERE user_id=$1 ORDER BY created_at",
    [user_id]
  );
...
module.exports.delete_ = async (user_id, id, pool) => {
  let review = await pool.query(
    "DELETE FROM REVIEWS WHERE user_id=$1 AND id=$2 RETURNING *",
    [user_id, id]
  );
